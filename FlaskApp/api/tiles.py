...
def get_tile_data_definition(tile_id, timespan=None):
    ...
    if tile_id == "meraki_device_summary":
        ...
        dev_stat = dashboard.organizations.getOrganizationDevicesStatuses(in_list[0], total_pages=10)
        ...
        dev_stat = dashboard.organizations.getOrganizationDevices(in_list[0])
        ...
        dev_stat = dashboard.organizations.getOrganizationDevicesStatuses(in_list[0], total_pages=10)
        dev_list = dashboard.organizations.getOrganizationDevices(in_list[0])
        ...
        dev_stat = dashboard.appliance.getOrganizationApplianceSecurityEvents(in_list[0], total_pages=10, sortOrder="descending", perPage=1000, t0=start_time, t1=end_time)
        ...
        end_time = ct.isoformat() + 'Z'
        dev_stat = dashboard.appliance.getOrganizationApplianceSecurityEvents(in_list[0], total_pages=2, sortOrder="descending", perPage=1000, t0=start_time, t1=end_time)
...
@tiles_api.route('/tiles', methods=['POST'])
def get_tiles():
    tiles = []
...
@tiles_api.route('/tiles/tile', methods=['POST'])
def get_tiles_tile():
...
@tiles_api.route('/tiles/tile-data', methods=['POST'])
def get_tile_data():
    json_data = request.json
    tid = json_data.get("tile_id")
